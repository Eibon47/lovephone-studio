...
部署后的 LovePhone 会同时发布 `/.netlify/functions/music`。用户在“设置 -> 音乐服务”开启在线音乐后，默认就会使用这个同域音乐网关，不需要填写外部 API 地址。

- 网关只开放搜索、推荐、榜单、歌单、歌词、播放地址和二维码登录所需的固定接口。
- 网易云登录 Cookie 只保存在用户自己的浏览器 IndexedDB；每次请求才传给函数，函数不会保存 Cookie，也不会把它写入日志或响应头。
- 用户仍可填写自己的 HTTPS 兼容 API 地址作为备用服务。
- 音乐内容、播放授权、歌词、封面和账号数据仍受网易云服务条款及相关版权约束；公开发布前应完成授权核验。
...
确认访问的是 Netlify 的 `https://` 地址，而不是电脑局域网的 `http://192.168.x.x:5178` 地址。局域网预览不会包含 Netlify 的 AI 与内置音乐函数。
